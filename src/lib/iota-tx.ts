...
function asU64(value: bigint | string) {
...
export function buildCreateInvoiceTx(
  ...
  tx.moveCall({
    ...
    arguments: [
      ...
      tx.object(notarizationId),
      tx.pure.vector("u8", hexToBytes(invoiceHashHex)),
      tx.pure.u64(asU64(amountNanos)),
...
export function buildListForFundingTx(
  packageId: string,
  invoiceId: string,
  discountPriceNanos: bigint | string,
) {
  const tx = new Transaction();
  tx.moveCall({
    target: moveTarget(packageId, "list_for_funding"),
    arguments: [tx.object(invoiceId), tx.pure.u64(asU64(discountPriceNanos))],
  });
  return tx;
}
...
export function buildFundTx(
  ...
  const tx = new Transaction();
  const [payment] = tx.splitCoins(tx.gas, [tx.pure.u64(asU64(discountPriceNanos))]);
  tx.moveCall({
...
export function buildRepayTx(
  ...
  const tx = new Transaction();
  const [payment] = tx.splitCoins(tx.gas, [tx.pure.u64(asU64(amountNanos))]);
  tx.moveCall({
...
export function buildCancelTx(
  ...
  tx.moveCall({
    ...
    arguments: [tx.object(registryId), tx.object(invoiceId)],
  });
  return tx;
}
